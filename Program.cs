...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        //captação da decisão do usuário
        ...
        if (opcao == 1)
        {
            ...
            try
            {
                ...
                for (int i = 2; i <= totalLinhas; i++)
                {
                    ...
                    var status = planilha.Cell($"C{i}").Value.ToString();

                    Cadastros novoCadastro = new Cadastros { Nome = NomePessoa, Idade = idade, Status = status};
